...
final class AllDepartures extends PowerGridComponent
{
    use ActionButton;
    ...
    */
    ...
    {
        $bookings = Booking::whereIn('ticket_status', ['issued', 'Booked'])->latest()->get();
        $array = [];
        foreach ($bookings as $booking) {
            if (Carbon::parse(str_replace('T', ' ', json_decode($booking->routes)->itineraries[0]->segments[0]->departure->at))->format('Y-m-d') < now()->format('Y-m-d') || $booking->ticket_status == "Cancel" || isset(json_decode($booking->routes)->itineraries[0]->segments[0]->departure->at) && Carbon::parse(str_replace('T', ' ', json_decode($booking->routes)->itineraries[0]->segments[0]->departure->at))->diffInHours(now()) > 72) {
                // array_push($array, $booking->id);
            } else {
                array_push($array, $booking->id);
            }
            if ($booking->trip_type == 'return' && isset(json_decode($booking->routes)->itineraries[1])) {
                if (Carbon::parse(str_replace('T', ' ', json_decode($booking->routes)->itineraries[1]->segments[0]->departure->at))->format('Y-m-d') < now()->format('Y-m-d') || $booking->ticket_status == "Cancel" || isset(json_decode($booking->routes)->itineraries[1]->segments[0]->departure->at) && Carbon::parse(str_replace('T', ' ', json_decode($booking->routes)->itineraries[1]->segments[0]->departure->at))->diffInHours(now()) > 72) {
                    // array_push($array, $booking->id);
                } else {
                    array_push($array, $booking->id);
                }
        ...
        return Booking::query()->whereIn('id', $array)->latest();
    }
